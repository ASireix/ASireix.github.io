...
(function($, window, document) {
  ...
  // Extend function for deep copying objects
  function extend(out) {
    out = out || {};
  ...
  function Plugin(element, options) {
    ...
    /**
     * Setter: particle stacking position (its "layer")
     */
  ...
  window[pluginName] = function(elements, options) {
    var ret = [];
    for (var i = 0; i < elements.length; i++) {
      if (typeof options === 'object' || !options) {
        if (!elements[i].plugin_particleground) {
          elements[i].plugin_particleground = new Plugin(elements[i], options);
        }
      } else if (elements[i].plugin_particleground[options]) {
        ret.push(elements[i].plugin_particleground[options].apply(elements[i].plugin_particleground, Array.prototype.slice.call(arguments, 2)));
      }
    }
    return ret;
  };
...
export default $.fn.particleground;
